...
public class OutOfScopeHandlerClient implements AutoCloseable {
    ...
    private final OosHandlerGrpc.OosHandlerBlockingStub blockingStub;

    private final OosHandlerGrpc.OosHandlerStub asyncStub;
    ...
    public OutOfScopeHandlerClient(ManagedChannelBuilder<?> channelBuilder) {
        ...
        blockingStub = OosHandlerGrpc.newBlockingStub(channel);
        asyncStub = OosHandlerGrpc.newStub(channel);
    }
    ...
    public void submitUri(QueuedUri uri) {
        try {
            ...
            GrpcUtil.forkedCall(() -> blockingStub.submitUri(request));
